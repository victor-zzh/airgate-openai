...
export function isUsageServiceTierFast(context: UsageRecordSurfaceProps['context']): boolean {
  ...
  return (
    <div className="flex shrink-0 gap-1">
      {chips.map((chip) => (
        <span
          key={chip.label}
          className={`inline-flex shrink-0 items-center rounded px-1.5 font-semibold leading-4 whitespace-nowrap ${chip.dotColor ? 'ag-usage-image-size-chip justify-start gap-1 text-[11px]' : 'text-[12px]'} ${chip.fastMark ? 'ag-usage-fast-marked-chip gap-1' : ''}`}
        ...
        >
          ...
          {chip.fastMark ? <span className="ag-usage-fast-indicator" aria-hidden="true" /> : null}
          {chip.label}
        </span>
      ))}
    </div>
